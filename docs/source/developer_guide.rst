...
Add a blank line after the first line to add a more detailed description if needed. The detailed description can be multiple lines and starts with a capital letter.
...
We also use Atomic Commits. This means that each commit should only contain changes related to one feature or bug fix. There's nothing wrong with having multiple commits for a single feature or bug fix or committing only a few lines of code.
This makes it easier to understand the history of the project and to revert changes if necessary.
...
In addition, this package is typed statically with `PEP 484 <https://peps.python.org/pep-0484/>`_ type hints.
`mypy <https://mypy.readthedocs.io/en/stable/>`_ is used for type checking. Please run ``mypy src`` to check the code for type errors.
Several linters are implemented via pre-commit hooks. Please install pre-commit and run ``pre-commit install`` to enable them. You can run the hooks manually with ``pre-commit run --all-files``. This includes the linting, formatting and type checking as well.
...
This will perform all tests under different Python versions in their own isolated environments.
Coverage is collected from all workers and combined afterwards.
Additionally, pre-commit hooks for linting, formatting and type checking are run.
Finally the documentation is built as html.
